...
router.get('/', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `${LIST_QUERY} ${where} ORDER BY l.created_at DESC LIMIT ${limitParam} OFFSET ${offsetParam}`,
      params
    );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    // value) so it matches node_log to microsecond precision — see the note
    // in ingest.js about why a JS-Date round-trip would truncate and miss.
    await client.query(
