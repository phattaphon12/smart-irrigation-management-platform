...
apiKey/applicationKey/station MAC ตั้งค่าที่ **`sugarcane-backend`** เท่านั้น (ไม่ใช่ frontend `.env`
อีกต่อไป) — `src/lib/api/ambientWeatherClient.js` ฝั่ง frontend เรียกผ่าน backend proxy เสมอ ทำให้
browser ไม่เห็น apiKey/applicationKey จริงเลย (เทียบกับตอนเรียก Ambient Weather ตรงๆ ที่ key จะโผล่ใน
network tab)

1. ตั้งค่าที่ backend (ดู `sugarcane-backend/README` หรือ `.env.example` ของ backend): คัดลอก
   `.env.example` เป็น `.env` แล้วใส่ `AMBIENT_API_KEY`, `AMBIENT_APPLICATION_KEY`,
   `AMBIENT_STATION_MAC` — หรือใส่/แก้ทีหลังได้จากหน้า **⚙️ Settings** ในแอปโดยไม่ต้อง restart
   backend (ค่าจะถูกเก็บลง `sugarcane-backend/data/ambient-config.json`)
2. รัน backend: `cd sugarcane-backend && npm run dev` (default port 4000)
3. ฝั่ง frontend ตั้ง `VITE_BACKEND_URL` (default `http://localhost:4000`) และ
   `VITE_USE_LIVE_WEATHER=true` ใน `.env`
4. `npm run dev`

Backend proxy จัดการตามกติกาที่เอกสารระบุไว้แล้ว:
